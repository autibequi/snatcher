...
type TabKey = 'category' | 'brand' | 'pending'
...
export default function Taxonomy() {
  ...
  const queryKey = tab === 'pending' ? ['taxonomy', 'pending'] : ['taxonomy', tab]
  const url = tab === 'pending' ? '/api/taxonomy/pending' : `/api/taxonomy?type=${tab}`

  const { data: items = [], isLoading } = useQuery<TaxonomyEntry[]>({
    queryKey,
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex gap-1 border-b border-border mb-4">
        {(['category', 'brand', 'pending'] as TabKey[]).map(t => (
          <button
            key={t}
            onClick={() => setTab(t)}
          ...
          >
            {t === 'category' ? 'Categorias' : t === 'brand' ? 'Marcas' : 'Pendentes'}
          </button>
        ))}
      </div>
                  ...
                  <tr key={item.id} className="border-b border-border last:border-0 hover:bg-surface-2/50">
                    ...
                    <td className="px-4 py-2 text-fg-2">
                      <div className="flex flex-wrap gap-1">
                        {item.keywords.slice(0, 5).map((k, i) => (
