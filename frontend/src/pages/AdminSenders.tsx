...
export default function AdminSenders() {
  ...
  return (
    <div className={`${pageContainer} space-y-6`}>
      ...
      {/* Banner */}
      <div className="bg-accent-soft border border-blue-200 rounded-lg px-4 py-3 text-sm text-accent">
      ...
      {!loading && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {modems.map(modem => {
            ...
            return (
              <div key={modem.slug} className="bg-surface border border-border rounded-lg p-4 shadow-sm flex flex-col gap-3">
                {/* Title row */}
                <div className="flex items-center justify-between">
                  <span className="font-semibold text-fg">{modem.slug}</span>
                ...
                <div className="text-sm text-fg-2 space-y-1">
                  <div className="flex justify-between">
                    <span className="text-fg-4">Queue pending</span>
                  ...
                  <div className="flex justify-between">
                    <span className="text-fg-4">Ultimo envio</span>
                  ...
                  {modem.paused_reason && (
                    <div className="flex justify-between">
                      <span className="text-fg-4">Motivo</span>
                ...
                {isExpanded && modemAccounts.length > 0 && (
                  <div className="border-t pt-3 space-y-2">
                    {modemAccounts.map(acc => (
                      <div key={acc.id} className="text-xs text-fg-2 flex flex-col gap-0.5 border-b pb-2 last:border-0">
                        <div className="flex items-center justify-between">
                ...
                {isExpanded && modemAccounts.length === 0 && (
                  <p className="text-xs text-fg-4 border-t pt-2">Nenhuma conta vinculada.</p>
                )}
              </div>
            )
          })}
          ...
          {modems.length === 0 && (
            <p className="text-fg-3 text-sm col-span-3">Nenhum modem encontrado.</p>
          )}
        </div>
      )}
    </div>
